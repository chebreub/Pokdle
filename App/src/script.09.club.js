...
function renderCatalogPicks(category, query) {
  ...
  panel.innerHTML = '<div class="club-section-head"><h3>Commence ici</h3><span>Trois incontournables</span></div><div class="club-picks-grid">' +
    picks.map((p, i) => '<button type="button" class="club-pick club-pick-' + i + '" data-action="openFromAllModes" data-args="' + escapeHtml(JSON.stringify([p[0]])) + '"><span class="club-pick-icon" aria-hidden="true"><svg><use href="#i-' + p[4] + '"/></svg></span><small>' + p[1] + '</small><b>' + p[2] + '</b><span>' + p[3] + '</span><strong aria-hidden="true">Jouer →</strong></button>').join("") + '</div><p class="club-more-label">Ou explore les autres jeux ci-dessous</p>';
}
