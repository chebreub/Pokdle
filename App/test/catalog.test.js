...
const cards = [...html.matchAll(/<button[^>]*class="card all-modes-card"[\s\S]*?<\/button>/g)].map(([markup]) => ({dataset:{category:markup.match(/data-category="([^"]+)"/)?.[1]}, textContent:markup.replace(/<[^>]+>/g,' '), markup}));
function fixture() {
 const elements = Object.fromEntries(['mode-search','mode-empty','mode-results','mode-hub-description','home-gens-card'].map(id => [id,{value:'',hidden:false,focus(){}}]));
 ...
 vm.createContext(context);vm.runInContext(read('src/script.08.catalog.js'),context);return {context,elements,rows};
}
...
test('changing category clears stale searches and saves it for Back',()=>{
 const {context,elements}=fixture();elements['mode-search'].value='quiz';context.setModeCatalogCategory('friends');assert.equal(elements['mode-search'].value,'');assert.equal(context.history.state.category,'friends');
});
